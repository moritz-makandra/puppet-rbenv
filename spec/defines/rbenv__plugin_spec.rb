...
describe 'rbenv::plugin', type: :define do
  ...
  it 'clones repository to the right path' do
    is_expected.to contain_exec("rbenv::plugin::checkout #{user} #{plugin_name}").with(
      command: "git clone #{source} #{target_path}",
      user: user,
      creates: target_path,
      require: %r{rbenv::plugins #{user}},
      path: ['/bin', '/usr/bin', '/usr/sbin'],
    )
  end

  it 'pulls the latest plugin changes from their git repos' do
    is_expected.to contain_exec("rbenv::plugin::update #{user} #{plugin_name}").with(
      command: 'git pull',
      user: user,
      cwd: target_path,
      require: %r{rbenv::plugin::checkout #{user} #{plugin_name}},
      path: ['/bin', '/usr/bin', '/usr/sbin'],
  ...
  context 'with source != git' do
    ...
    it 'fails informing that it is not supported yet' do
      expect {
        is_expected.to contain_exec("rbenv::plugin::checkout #{user} #{plugin_name}")
      }.to raise_error(Puppet::Error, %r{Only git plugins are supported})
    end
  end
end
